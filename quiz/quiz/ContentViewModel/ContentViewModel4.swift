...
class ContentViewModel4: ObservableObject {
    ...
    @Published var quizData = quizmanager4.shared.makeQuizData()
        ...
        if let index = quizData.optionsList.firstIndex(where: { $0.optionId == selectedOption.optionId }) {
            ...
            if selectedOption.optionId ==  quizData.answer {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    if quizmanager4.shared.isQuizEnded {
                        quizmanager4.shared.isQuizCompleted = true
                        quizmanager4.shared.isQuizWon = true
                        
                        self.reset()
                        
                    } else {
                        self.quizData = quizmanager4.shared.makeQuizData()
    ...
    func start() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats:true) { time in
            if self.progress == self.maxProgress {
                quizmanager4.shared.isQuizCompleted = true
                quizmanager4.shared.isQuizWon = false
                self.reset()
            } else {
                self.progress += 1
            }
    ...
    /// ゲームをもう一度行う
    func restart() {
        quizmanager4.shared.quizReset()
        quizData = quizmanager4.shared.makeQuizData()
        self.start()
    }
