...
class ContentViewModel2: ObservableObject {
    ...
    @Published var quizData = quizmanager2.shared.makeQuizData()
        ...
        if let index = quizData.optionsList.firstIndex(where: { $0.optionId == selectedOption.optionId }) {
            ...
            if selectedOption.optionId ==  quizData.answer {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    if quizmanager2.shared.isQuizEnded {
                        quizmanager2.shared.isQuizCompleted = true
                        quizmanager2.shared.isQuizWon = true
                        
                        self.reset()
                        
                    } else {
                        self.quizData = quizmanager2.shared.makeQuizData()
    ...
    func start() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats:true) { time in
            if self.progress == self.maxProgress {
                quizmanager2.shared.isQuizCompleted = true
                quizmanager2.shared.isQuizWon = false
                self.reset()
            } else {
                self.progress += 1
            }
    ...
    /// ゲームをもう一度行う
    func restart() {
        quizmanager2.shared.quizReset()
        quizData = quizmanager2.shared.makeQuizData()
        self.start()
    }
